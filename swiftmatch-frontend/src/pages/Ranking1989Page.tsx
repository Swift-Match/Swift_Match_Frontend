...
const a1989_ALBUM_ID = 5; // ajuste se necessário
...
const a1989_DARK = '#034A62';   // dark
const a1989_LIGHT = '#D1F3FF';  // light
...
const LogoSVG: React.FC<{ color?: string; height?: number }> = ({ color = a1989_LIGHT, height = 40 }) => (
  <svg
...
const Ranking1989Page: React.FC = () => {
  ...
  const fetchTracks = useCallback(async () => {
    ...
    const url = `${API_BASE_URL}/tracks/album/${a1989_ALBUM_ID}/`;
  ...
  const handleSaveRanking = async () => {
    ...
    const payload = {
        album_id: a1989_ALBUM_ID,
        rankings: rankingsPayload
    };

    const url = `${API_BASE_URL}/rankings/tracks/${a1989_ALBUM_ID}/`;
  ...
  const renderTrackList = () => {
    if (isLoading) {
      return <div style={{ textAlign: 'center', padding: 40, color: a1989_DARK }}>Carregando a lista de músicas do Speak Now...</div>;
    }

    if (error) {
      return <div style={{ textAlign: 'center', padding: 40, color: a1989_DARK, fontWeight: 'bold' }}>Erro: {error}</div>;
    }

    if (tracks.length === 0) {
      return <div style={{ textAlign: 'center', padding: 40, color: a1989_DARK }}>Nenhuma música encontrada para este álbum.</div>;
    }
              ...
              <li key={track.id} style={{ margin: 0 }}>
                <button
                  ...
                  style={{
                    ...
                    padding: '10px',
                    borderRadius: 10,
                    border: `1px solid ${a1989_DARK}30`,
                    background: a1989_DARK,
                    color: a1989_DARK,
                    cursor: 'grab',
                    textAlign: 'left',
                ...
                {dragOverItemIndex.current === index && dragItemIndex.current !== index && (
                  <div style={{ height: 11, background: a1989_DARK, borderRadius: 2, margin: '4px 0', opacity: 0.5, transition: 'all 0.2s' }} />
                )}
              </li>
            );
          })}
  ...
  return (
    <div style={{ width: '100vw', minHeight: '100vh', background: wrapperBg, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingBottom: 80, fontFamily: '"Inter", sans-serif' }}>
      ...
      >
        <LogoSVG color={a1989_LIGHT} height={40} />
      </header>

      <div style={{ height: 120, width: '100%' }} />

      <h1 style={{ color: a1989_DARK, fontSize: 40, fontWeight: 800, marginBottom: 12 }}>
        1989 Ranking
      </h1>

      <p style={{ color: a1989_DARK, marginBottom: 40, fontSize: 18, textAlign: 'center' }}>
...
export default Ranking1989Page;
