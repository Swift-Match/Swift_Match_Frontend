...
const SHOWGIRL_ALBUM_ID = 12; // ajuste se necessário
...
const SHOWGIRL_DARK = '#C44615';   // dark
const SHOWGIRL_LIGHT = '#9CCBC3';  // light
...
const ButtonCustom: React.FC<ButtonCustomProps> = ({
  ...
  height = '50px',
  borderRadius = 8,
  background = SHOWGIRL_DARK,
  color = SHOWGIRL_LIGHT,
  fontSize = 16,
  fontWeight = 700,
  onClick,
  children,
...
const LogoSVG: React.FC<{ color?: string; height?: number }> = ({ color = SHOWGIRL_LIGHT, height = 40 }) => (
  <svg
...
const RankingShowgirlPage: React.FC = () => {
  ...
  const fetchTracks = useCallback(async () => {
    ...
    const url = `${API_BASE_URL}/tracks/album/${SHOWGIRL_ALBUM_ID}/`;
  ...
  const handleSaveRanking = async () => {
    ...
    const payload = {
        album_id: SHOWGIRL_ALBUM_ID,
        rankings: rankingsPayload
    };

    const url = `${API_BASE_URL}/rankings/tracks/${SHOWGIRL_ALBUM_ID}/`;
  ...
  // layout vars
  const wrapperBg = SHOWGIRL_LIGHT;
  const fadeWidth = 120;
  ...
  const renderTrackList = () => {
    if (isLoading) {
      return <div style={{ textAlign: 'center', padding: 40, color: SHOWGIRL_DARK }}>Carregando a lista de músicas do The Life of a Showgirl...</div>;
    }

    if (error) {
      return <div style={{ textAlign: 'center', padding: 40, color: SHOWGIRL_DARK, fontWeight: 'bold' }}>Erro: {error}</div>;
    }

    if (tracks.length === 0) {
      return <div style={{ textAlign: 'center', padding: 40, color: SHOWGIRL_DARK }}>Nenhuma música encontrada para este álbum.</div>;
    }
              ...
              <li key={track.id} style={{ margin: 0 }}>
                <button
                  ...
                  style={{
                    ...
                    padding: '10px',
                    borderRadius: 10,
                    border: `1px solid ${SHOWGIRL_DARK}30`,
                    background: SHOWGIRL_DARK,
                    color: SHOWGIRL_DARK,
                    cursor: 'grab',
                    textAlign: 'left',
                ...
                {dragOverItemIndex.current === index && dragItemIndex.current !== index && (
                  <div style={{ height: 11, background: SHOWGIRL_DARK, borderRadius: 2, margin: '4px 0', opacity: 0.5, transition: 'all 0.2s' }} />
                )}
              </li>
            );
          })}
  ...
  return (
    <div style={{ width: '100vw', minHeight: '100vh', background: wrapperBg, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingBottom: 80, fontFamily: '"Inter", sans-serif' }}>
      ...
      >
        <LogoSVG color={SHOWGIRL_LIGHT} height={40} />
      </header>

      <div style={{ height: 120, width: '100%' }} />

      <h1 style={{ color: SHOWGIRL_DARK, fontSize: 40, fontWeight: 800, marginBottom: 12 }}>
        The Life of a Showgirl Ranking
      </h1>

      <p style={{ color: SHOWGIRL_DARK, marginBottom: 40, fontSize: 18, textAlign: 'center' }}>
...
export default RankingShowgirlPage;
