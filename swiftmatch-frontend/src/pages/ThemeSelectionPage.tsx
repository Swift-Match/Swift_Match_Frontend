...
interface ThemeColorMap { [key: string]: ThemeColor }
...
const themeImages: { [key: string]: string } = {
  TS: '/ThemeSelectionAlbums/theme_taylorswift.png',
  FEARLESS: '/ThemeSelectionAlbums/theme_fearless.png',
  SPEAK_NOW: '/ThemeSelectionAlbums/theme_speaknow.png',
  RED: '/ThemeSelectionAlbums/theme_red.png',
  '1989': '/ThemeSelectionAlbums/theme_1989.png',
  REPUTATION: '/ThemeSelectionAlbums/theme_reputation.png',
  LOVER: '/ThemeSelectionAlbums/theme_lover.png',
  FOLKLORE: '/ThemeSelectionAlbums/theme_folklore.png',
  EVERMORE: '/ThemeSelectionAlbums/theme_evermore.png',
  MIDNIGHTS: '/ThemeSelectionAlbums/theme_midnights.png',
  TTPD: '/ThemeSelectionAlbums/theme_ttpd.png',
  SHOWGIRL: '/ThemeSelectionAlbums/theme_showgirl.png',
};

const ARROW_LEFT_IMAGE = 'Components/left_arrow.png'; 
const ARROW_RIGHT_IMAGE = '/Components/right_arrow.png';
...
const ThemeSelectionPage: React.FC = () => {
  ...
  const [itemPx, setItemPx] = useState<number>(120);
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        const res = await API.get('/api/users/themes/');
        const data: ThemeOption[] = res.data && res.data.length ? res.data : Object.keys(themeColorMap).map(k => ({ key: k, label: k }));
        setThemes(data);
        setActiveIndex(0);
      } catch {
        const fallback = Object.keys(themeColorMap).map(k => ({ key: k, label: k }));
        setThemes(fallback);
        setActiveIndex(0);
      } finally {
        setLoading(false);
  ...
  const recalc = useCallback(() => {
    ...
    const available = Math.max(0, w - totalGaps);
    const size = Math.floor(available / VISIBLE_COUNT);
    setItemPx(size);
  }, []);
...
  }, [recalc, themes.length]);
  ...
  const rotate = (dir: 'left' | 'right') => {
    ...
    setActiveIndex(prev => dir === 'right' ? (prev + 1) % themes.length : (prev - 1 + themes.length) % themes.length);
  };

  const handleSave = async () => {
    const cur = themes[activeIndex];
    if (!cur) return;
    setMessage('Salvando...');
    try {
      await API.patch('/api/users/me/theme/', { tema: cur.key });
      localStorage.setItem('userThemeKey', cur.key);
      navigate('/album-ranking');
    } catch {
  ...
  if (!themes.length) return <div className="h-screen flex items-center justify-center">Sem temas</div>;

  const currentThemeKey = themes[activeIndex].key;
  const indicatorHeight = itemPx;
  const wrapperBg = themeColorMap[currentThemeKey]?.light ?? '#fff';
  ...
  return (
    <div style={{ width: '100vw', minHeight: '100vh', background: wrapperBg, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <style>{`
                ...
                .side-arrow { width: 56px; height: 56px; border-radius: 999px; border: none; background:transparent; cursor: pointer; display:flex; align-items:center; justify-content:center; box-shadow: none; }
                .side-arrow:focus {
                    outline: none; 
                }
                .side-arrow:active { transform: scale(0.85); background: transparent; }
            `}</style>

            <header style={{ 
                width: '100%', 
                height: HEADER_HEIGHT, 
                background: themeColorMap[currentThemeKey].dark,
                display: 'flex',  
                alignItems: 'center', 
                paddingLeft: 80, 
                ...
                <LogoSVG color={themeColorMap[currentThemeKey].light} /> 
            </header>
        ...
        <p style={{ 
            ...
            Now, choose your favorite era
        </p>
    </div>

      {/* NEW: container with arrows outside (left/right) */}
      <div style={{ width: '100%', maxWidth: COMPONENT_MAX_WIDTH + 120 /* allow arrows area */, margin: '0 auto', padding: '140px 0', boxSizing: 'border-box', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 80 }}>
        {/* LEFT ARROW (outside) */}
        <button aria-label="Previous era" onClick={() => rotate('left')} className="side-arrow" style={{ marginLeft: -8 }}>
          <img 
...
        </button>
        ...
        >
          {/* left fade */}
          <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: fadeWidth, pointerEvents: 'none', zIndex: 50, background: `linear-gradient(to right, ${wrapperBg} 0%, rgba(255,255,255,0) 70%)`, opacity: 0.95 }} />
          {/* right fade */}
          <div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: fadeWidth, pointerEvents: 'none', zIndex: 50, background: `linear-gradient(to left, ${wrapperBg} 0%, rgba(255,255,255,0) 70%)`, opacity: 0.95 }} />

          {/* inner row
              NOTE: aumentei o padding vertical aqui também para centrar melhor os itens e evitar corte quando translateY é negativo
          */}
          <div style={{ display: 'flex', justifyContent: 'flex-start', gap: `${GAP_PX}px`, alignItems: 'center', padding: `${Math.max(12, extraVertical)}px 12px`, transform: `translateX(${rowShift}px)`, transition: 'transform 280ms cubic-bezier(.2,.9,.3,1)' }}>
            {VISIBLE_OFFSETS.map((off) => {
              const idx = getIndexOffset(activeIndex, off);
              const theme = themes[idx];
              const dist = Math.abs(off);
              const baseScale = dist === 0 ? 1.12 : dist === 1 ? 1.02 : dist === 2 ? 0.92 : 0.78;
              const visualScale = baseScale * ALBUM_SCALE;
              const translateY = dist === 0 ? '-6%' : '0%';
              const opacity = dist === 0 ? 1 : dist === 1 ? 0.95 : dist === 2 ? 0.85 : 0.6;
              const zIndex = 50 - dist;

              return (
                <div key={`${theme.key}-${off}`} style={{
                    width: itemPx,
                    height: indicatorHeight,
                    borderRadius: dist === 0 ? '50%' : 12, 
                    overflow: 'visible',
        ...
        {/* RIGHT ARROW (outside) */}
        <button aria-label="Next era" onClick={() => rotate('right')} className="side-arrow" style={{ marginRight: -8 }}>
          <img 
...
        </button>
      ...
      {/* CTA next */}
      <div style={{ width: '100%', maxWidth: COMPONENT_MAX_WIDTH, margin: '8px auto 32px', display: 'flex', justifyContent: 'flex-end', padding: '0 12px', boxSizing: 'border-box', position: 'relative'  }}>
        <button onClick={handleSave} style={{ width: 150, height: 55, borderRadius: 8, background: themeColorMap[currentThemeKey].dark, color: themeColorMap[currentThemeKey].light, fontWeight: 600,  position: 'relative',  top: '-200px',  right: '-220px' }}>
