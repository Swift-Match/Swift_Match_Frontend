...
const FOLKLORE_DARK = '#000000';   
const FOLKLORE_LIGHT = '#D5D5D5';  
...
const ButtonCustom: React.FC<ButtonCustomProps> = ({
  ...
  height = '50px',
  borderRadius = 8,
  background = FOLKLORE_DARK,
  color = FOLKLORE_LIGHT,
  fontSize = 16,
  fontWeight = 700,
  onClick,
  children,
...
const LogoSVG: React.FC<{ color?: string; height?: number }> = ({ color = FOLKLORE_LIGHT, height = 40 }) => (
  <svg
...
const RankingFolklorePage: React.FC = () => {
  ...
  const fetchTracks = useCallback(async () => {
    ...
    const url = `${API_BASE_URL}/tracks/album/${FOLKLORE_ALBUM_ID}/`;
  ...
  const handleSaveRanking = async () => {
    ...
    const payload = {
        album_id: FOLKLORE_ALBUM_ID,
        rankings: rankingsPayload
    };

    const url = `${API_BASE_URL}/rankings/tracks/${FOLKLORE_ALBUM_ID}/`;
  ...
  const wrapperBg = FOLKLORE_LIGHT;
  const fadeWidth = 120;
  ...
  const renderTrackList = () => {
    ...
    if (error) {
      return <div style={{ textAlign: 'center', padding: 40, color: FOLKLORE_DARK, fontWeight: 'bold' }}>Erro: {error}</div>;
    }

    if (tracks.length === 0) {
      return <div style={{ textAlign: 'center', padding: 40, color: FOLKLORE_DARK }}>Nenhuma música encontrada para este álbum.</div>;
    }
              ...
              <li key={track.id} style={{ margin: 0 }}>
                <button
                  ...
                  style={{
                    ...
                    padding: '10px',
                    borderRadius: 10,
                    border: `1px solid ${FOLKLORE_DARK}30`,
                    background: FOLKLORE_DARK,
                    color: FOLKLORE_DARK,
                    cursor: 'grab',
                    textAlign: 'left',
                ...
                {dragOverItemIndex.current === index && dragItemIndex.current !== index && (
                  <div style={{ height: 11, background: FOLKLORE_DARK, borderRadius: 2, margin: '4px 0', opacity: 0.5, transition: 'all 0.2s' }} />
                )}
              </li>
            );
          })}
  ...
  return (
    <div style={{ width: '100vw', minHeight: '100vh', background: wrapperBg, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingBottom: 80, fontFamily: '"Inter", sans-serif' }}>
      ...
      >
        <LogoSVG color={FOLKLORE_LIGHT} height={40} />
      </header>

      <div style={{ height: 120, width: '100%' }} />

      <h1 style={{ color: FOLKLORE_DARK, fontSize: 40, fontWeight: 800, marginBottom: 12 }}>
        folklore Ranking
      </h1>

      <p style={{ color: FOLKLORE_DARK, marginBottom: 40, fontSize: 18, textAlign: 'center' }}>
...
export default RankingFolklorePage;
