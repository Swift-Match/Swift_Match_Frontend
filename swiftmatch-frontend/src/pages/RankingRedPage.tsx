...
const LogoSVG: React.FC<{ color?: string; height?: number }> = ({ color = RED_LIGHT, height = 40 }) => (
  <svg
...
const RankingRedPage: React.FC = () => {
  ...
  const fetchTracks = useCallback(async () => {
    ...
    const url = `${API_BASE_URL}/tracks/album/${RED_ALBUM_ID}/`;
  ...
  const handleSaveRanking = async () => {
    ...
    const url = `${API_BASE_URL}/rankings/tracks/${RED_ALBUM_ID}/`;
  ...
  const renderTrackList = () => {
    ...
    if (error) {
      return <div style={{ textAlign: 'center', padding: 40, color: RED_DARK, fontWeight: 'bold' }}>Erro: {error}</div>;
    }

    if (tracks.length === 0) {
      return <div style={{ textAlign: 'center', padding: 40, color: RED_DARK }}>Nenhuma música encontrada para este álbum.</div>;
    }
            ...
            return (
              <li key={track.id} style={{ margin: 0 }}>
                ...
                {dragOverItemIndex.current === index && dragItemIndex.current !== index && (
                  <div style={{ height: 11, background: RED_DARK, borderRadius: 2, margin: '4px 0', opacity: 0.5, transition: 'all 0.2s' }} />
                )}
              </li>
            );
          })}
  ...
  return (
    <div style={{ width: '100vw', minHeight: '100vh', background: wrapperBg, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingBottom: 80, fontFamily: '"Inter", sans-serif' }}>
      ...
      >
        <LogoSVG color={RED_LIGHT} height={40} />
      </header>

      <div style={{ height: 120, width: '100%' }} />

      <h1 style={{ color: RED_DARK, fontSize: 40, fontWeight: 800, marginBottom: 12 }}>
        Red Ranking
      </h1>

      <p style={{ color: RED_DARK, marginBottom: 40, fontSize: 18, textAlign: 'center' }}>
