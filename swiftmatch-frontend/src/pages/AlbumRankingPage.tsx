...
const HEADER_GUTTER = 24; // alinhamento do conteúdo do cabeçalho (logo, texto, etc.)
...
const albumColorMap: { [key: string]: string } = {
  ...
  'EVERMORE': '#E0C9AF',
  'SPEAK NOW': '#130A17',
  'THE TORTURED POETS DEPARTMENT': '#CACACA',
  '1989': '#D1F3FF',
  'TAYLOR SWIFT': '#0C1A0C',
  'THE LIFE OF A SHOWGIRL': '#9CCBC3',
  'MIDNIGHTS': '#1F2E43',
};
...
const getAlbumBgColor = (title: string) => {
  ...
  const keys = Object.keys(albumColorMap).sort((a, b) => b.length - a.length);

  for (const k of keys) {
    const pattern = `\\b${escapeRegExp(k)}\\b`;
    const re = new RegExp(pattern, 'i');
    if (re.test(txt)) return albumColorMap[k];
  }

  if (/\bTORTURED\b/i.test(txt) && /\bPOETS\b/i.test(txt)) return albumColorMap['THE TORTURED POETS DEPARTMENT'];
  if (/\bSHOWGIRL\b/i.test(txt)) return albumColorMap['THE LIFE OF A SHOWGIRL'];
  if (/\bTAYLOR\b/i.test(txt)) return albumColorMap['TAYLOR SWIFT'];

  return '#FFFFFF';
};

// --- image map (public/AlbumRanking) ---
const albumImageMap: { [key: string]: string } = {
  '1989': '/AlbumRanking/AlbumRanking1989.png',
  'EVERMORE': '/AlbumRanking/AlbumRankingEvermore.png',
  'FEARLESS': '/AlbumRanking/AlbumRankingFearless.png',
  'FOLKLORE': '/AlbumRanking/AlbumRankingFolklore.png',
  'LOVER': '/AlbumRanking/AlbumRankingLover.png',
  'MIDNIGHTS': '/AlbumRanking/AlbumRankingMidnights.png',
  'RED': '/AlbumRanking/AlbumRankingRed.png',
  'REPUTATION': '/AlbumRanking/AlbumRankingRep.png',
  'SHOWGIRL': '/AlbumRanking/AlbumRankingShowgirl.png',
  'SPEAK NOW': '/AlbumRanking/AlbumRankingSpeakNow.png',
  'TS': '/AlbumRanking/AlbumRankingTS.png',
  'TTPD': '/AlbumRanking/AlbumRankingTTPD.png',
};

// sizes (opcional)
type SizeEntry = { maxWidth?: string; maxHeight?: number };
const albumImageSizeMap: { [key: string]: SizeEntry } = {
  '1989': { maxWidth: '62%', maxHeight: 99 },
  'EVERMORE': { maxWidth: '58%', maxHeight: 60 },
  'FEARLESS': { maxWidth: '56%', maxHeight: 60 },
  'FOLKLORE': { maxWidth: '60%', maxHeight: 100 },
  'LOVER': { maxWidth: '66%', maxHeight: 90 },
  'MIDNIGHTS': { maxWidth: '60%', maxHeight: 60 },
  'RED': { maxWidth: '52%', maxHeight: 74 },
  'REPUTATION': { maxWidth: '64%', maxHeight: 90 },
  'SHOWGIRL': { maxWidth: '70%', maxHeight: 90 },
  'SPEAK NOW': { maxWidth: '60%', maxHeight: 90 },
  'TS': { maxWidth: '80%', maxHeight: 127 },
  'TTPD': { maxWidth: '62%', maxHeight: 120 },
};
const DEFAULT_IMAGE_SIZE: SizeEntry = { maxWidth: '70%', maxHeight: 72 };

const getAlbumImage = (title: string) => {
  const txt = (title || '').toString();
  const keys = Object.keys(albumImageMap).sort((a, b) => b.length - a.length);
  for (const k of keys) {
    const pattern = `\\b${escapeRegExp(k)}\\b`;
    const re = new RegExp(pattern, 'i');
    if (re.test(txt)) return albumImageMap[k];
  }
  if (/\bTORTURED\b/i.test(txt) && /\bPOETS\b/i.test(txt)) return albumImageMap['TTPD'];
  if (/\bSHOWGIRL\b/i.test(txt)) return albumImageMap['SHOWGIRL'];
  if (/\bTAYLOR\b/i.test(txt) || /\bTS\b/i.test(txt)) return albumImageMap['TS'];
  return '';
};

const getAlbumImageSize = (title: string): SizeEntry => {
  const txt = (title || '').toString();
  const keys = Object.keys(albumImageSizeMap).sort((a, b) => b.length - a.length);
  for (const k of keys) {
    const re = new RegExp(`\\b${escapeRegExp(k)}\\b`, 'i');
    if (re.test(txt)) return albumImageSizeMap[k];
  }
  if (/\bTORTURED\b/i.test(txt) && /\bPOETS\b/i.test(txt)) return albumImageSizeMap['TTPD'] || DEFAULT_IMAGE_SIZE;
  if (/\bSHOWGIRL\b/i.test(txt)) return albumImageSizeMap['SHOWGIRL'] || DEFAULT_IMAGE_SIZE;
  if (/\bTAYLOR\b/i.test(txt) || /\bTS\b/i.test(txt)) return albumImageSizeMap['TS'] || DEFAULT_IMAGE_SIZE;
...
const AlbumRankingPage: React.FC = () => {
  ...
  const handleSaveRanking = async () => {
    const formatted = ranking.map((album, i) => ({ album_id: album.id, position: i + 1 }));
    try {
  ...
  const handleDragStart = (e: React.DragEvent<HTMLDivElement>, pos: number) => {
    dragItem.current = pos;
    e.currentTarget.classList.add('opacity-50', 'ring-4');
    e.dataTransfer.effectAllowed = 'move';
    setTimeout(() => (e.currentTarget.style.display = 'none'), 0);
  };
  const handleDragEnter = (e: React.DragEvent<HTMLDivElement>, pos: number) => { e.preventDefault(); dragOverItem.current = pos; };
  const handleDragOver = (e: React.DragEvent<HTMLDivElement>) => e.preventDefault();
  const handleDragEnd = (e: React.DragEvent<HTMLDivElement>) => {
    e.currentTarget.classList.remove('opacity-50', 'ring-4');
  ...
  if (loading) {
    return (
      <div style={{ width: '100vw', minHeight: '100vh', background: colors.light, color: colors.dark }}>
        ...
        <div style={{ paddingTop: HEADER_HEIGHT + 40, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: `calc(100vh - ${HEADER_HEIGHT}px)` }}>
          <div className="text-2xl">Carregando seus álbuns...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ width: '100vw', minHeight: '100vh', backgroundColor: colors.light, color: textColor }}>
      <header style={{ width: '100%', height: HEADER_HEIGHT, background: colors.dark, display: 'flex', alignItems: 'center', paddingLeft: HEADER_GUTTER, justifyContent: 'flex-start', position: 'fixed', top: 0, zIndex: 100 }}>
        <LogoSVG color={colors.light} />
      </header>
          ...
          <div style={{ width: '100%', boxSizing: 'border-box' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {ranking.map((album, index) => {
                const bg = getAlbumBgColor(album.title);
                const fg = getContrastColor(bg);
                const imgPath = getAlbumImage(album.title);
                const imgSize = getAlbumImageSize(album.title);
                const imgSrc = imgPath || album.cover_image_url || '';
                ...
                if (imgSize.maxWidth) imgStyle.maxWidth = imgSize.maxWidth;
                if (imgSize.maxHeight) imgStyle.maxHeight = imgSize.maxHeight;
                ...
                return (
                  <div
                    ...
                    style={{
                      width: `calc(100% + ${HEADER_GUTTER * 2}px)`,
                      marginLeft: `-${HEADER_GUTTER}px`,
                  ...
                  >
                    <div style={{ width: '100%', maxWidth: `calc(100% - ${HEADER_GUTTER * 2}px)`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, boxSizing: 'border-box' }}>
                      <div style={{ width: 56, display: 'flex', alignItems: 'center', paddingLeft: HEADER_GUTTER }}>
                        <span style={{ fontWeight: 800, fontSize: 18, color: fg, whiteSpace: 'nowrap' }}>{index + 1}.</span>
                      </div>

                      <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', minWidth: 0 }}>
                        {imgSrc ? <img src={imgSrc} alt={album.title} style={imgStyle} /> : null}
                      </div>

                      <div style={{ width: 60, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: HEADER_GUTTER }}>
                        <div style={{ width: 60, height: 60, borderRadius: 6, backgroundImage: `url(${album.cover_image_url})`, backgroundSize: 'cover', backgroundPosition: 'center', flexShrink: 0 }} />
                      </div>
                    </div>
                  </div>
                );
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, padding: 16, display: 'flex', justifyContent: 'center', zIndex: 20 }}>
...
export default AlbumRankingPage;
