...
const REP_ALBUM_ID = 6; // ajuste se necessário
...
const REP_DARK = '#2A2628';   // dark
const REP_LIGHT = '#CACACA';  // light
...
const LogoSVG: React.FC<{ color?: string; height?: number }> = ({ color = REP_LIGHT, height = 40 }) => (
  <svg
...
const RankingRepPage: React.FC = () => {
  ...
  const fetchTracks = useCallback(async () => {
    ...
    const url = `${API_BASE_URL}/tracks/album/${REP_ALBUM_ID}/`;
  ...
  const handleSaveRanking = async () => {
    ...
    const payload = {
        album_id: REP_ALBUM_ID,
        rankings: rankingsPayload
    };

    const url = `${API_BASE_URL}/rankings/tracks/${REP_ALBUM_ID}/`;
  ...
  const renderTrackList = () => {
    if (isLoading) {
      return <div style={{ textAlign: 'center', padding: 40, color: REP_DARK }}>Carregando a lista de músicas do Reputation...</div>;
    }

    if (error) {
      return <div style={{ textAlign: 'center', padding: 40, color: REP_DARK, fontWeight: 'bold' }}>Erro: {error}</div>;
    }

    if (tracks.length === 0) {
      return <div style={{ textAlign: 'center', padding: 40, color: REP_DARK }}>Nenhuma música encontrada para este álbum.</div>;
    }
            ...
            return (
              <li key={track.id} style={{ margin: 0 }}>
                ...
                {dragOverItemIndex.current === index && dragItemIndex.current !== index && (
                  <div style={{ height: 11, background: REP_DARK, borderRadius: 2, margin: '4px 0', opacity: 0.5, transition: 'all 0.2s' }} />
                )}
              </li>
            );
          })}
  ...
  return (
    <div style={{ width: '100vw', minHeight: '100vh', background: wrapperBg, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingBottom: 80, fontFamily: '"Inter", sans-serif' }}>
      ...
      >
        <LogoSVG color={REP_LIGHT} height={40} />
      </header>

      <div style={{ height: 120, width: '100%' }} />

      <h1 style={{ color: REP_DARK, fontSize: 40, fontWeight: 800, marginBottom: 12 }}>
        Reputation Ranking
      </h1>

      <p style={{ color: REP_DARK, marginBottom: 40, fontSize: 18, textAlign: 'center' }}>
