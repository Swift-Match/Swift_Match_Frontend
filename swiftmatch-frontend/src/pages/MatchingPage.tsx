...
const themeColorMap: { [key: string]: ThemeColor } = {
  ...
  SPEAK_NOW: { dark: '#38182F', light: '#D6BADC' },
  ...
  FOLKLORE: { dark: '#2C2C2C', light: '#D5D5D5' },
...
const LogoSVG: React.FC<{ color: string }> = ({ color }) => (
  <svg viewBox="0 0 299 75" preserveAspectRatio="xMinYMid meet" xmlns="http://www.w3.org/2000/svg" style={{ height: 40, width: 'auto', display: 'block' }} fill={color}>
...
const decideEndpointFrom = (queryParams: Record<string, string>, passedState: any) => {
  const matchType = queryParams.matchType ?? passedState?.matchType;
  const albumId = queryParams.albumId ?? passedState?.albumId;
  const targetUserId = queryParams.targetUserId ?? passedState?.targetUserId;

  if (!targetUserId) return { error: 'missing_targetUserId' };

  if (matchType === 'tracks' || (matchType == null && albumId != null)) {
    if (!albumId) return { error: 'missing_albumId_for_tracks' };
    return {
      type: 'tracks',
      url: `${API_BASE}/api/rankings/compare/tracks/${targetUserId}/album/${albumId}/`,
      targetUserId,
  ...
  return {
    type: 'albums',
    url: `${API_BASE}/api/rankings/compare/albums/${targetUserId}/`,
    targetUserId,
    albumId: null,
  };
};
...
interface MatchingAnalysisProps {
  analysis: any;
  type: 'albums' | 'tracks';
  albumMap: Record<number, string>;
  trackMap: Record<number, string>;
  colors: ThemeColor;
}

const coverFilesByCode: Record<string, { front: string; original?: string }> = {
  TS: { front: 'TS_cover.jpg' },
  FEARLESS: { front: 'fearless_cover.jpg', original: 'fearless_original_cover.jpg' },
  SPEAK_NOW: { front: 'speakNow_cover.jpg', original: 'speakNow_original_cover.jpg' },
  RED: { front: 'red_cover.jpg', original: 'red_original_cover.jpg' },
  '1989': { front: '1989_cover.jpg', original: '1989_original_cover.jpg' },
  REPUTATION: { front: 'rep_cover.jpg' },
  LOVER: { front: 'lover_cover.jpg' },
  FOLKLORE: { front: 'folklore_cover.jpg' },
  EVERMORE: { front: 'evermore_cover.jpg' },
  MIDNIGHTS: { front: 'midnights_cover.jpg' },
  TTPD: { front: 'TTPD_cover.jpg' },
  SHOWGIRL: { front: 'Showgirl_cover.jpg' },
};

const publicCoverUrl = (filename: string | undefined) => {
...
const AlbumCoverBox: React.FC<{
  ...
  colors: ThemeColor;
}> = ({ albumTitle, normalizedCode, label, extraText, colors }) => {
  const mapping = normalizedCode ? coverFilesByCode[normalizedCode] : undefined;
  const frontUrl = mapping ? publicCoverUrl(mapping.front) : null;
  const originalUrl = mapping && mapping.original ? publicCoverUrl(mapping.original) : null;

  const isDouble = Boolean(originalUrl);
  ...
  const boxSize = isDouble ? { minWidth: 380, minHeight: 360, padding: 20 } : { minWidth: 320, minHeight: 280 };
  ...
  return (
    <div style={boxStyle}>
      {/* LABEL dentro: agora maior para ficar claro */}
      {label && <div style={{ fontSize: 20, fontWeight: 900, color: colors.light }}>{label}</div>}
      ...
      {/* album title e descrição/extra (tudo dentro) */}
      <div style={{ fontWeight: 900, fontSize: 22, marginTop: 6, color: colors.light }}>{albumTitle || 'Unknown'}</div>
      {extraText !== undefined && extraText !== null && (
        <div style={{ fontSize: 13, opacity: 0.9, color: colors.light }}>{String(extraText)}</div>
      )}
    </div>
  );
};

const MatchingAnalysisDisplay: React.FC<MatchingAnalysisProps> = ({ analysis, type, albumMap, trackMap, colors }) => {
  const mapToUse = type === 'albums' ? albumMap : trackMap;
  const idPrefix = type === 'albums' ? 'album' : 'track';

  const getName = (key: string) => {
    const id = analysis?.[key];
    if (id === undefined || id === null) return 'N/A';
    return mapToUse[id] || `ID ${id} (name not found)`;
  };

  const items = [
    { label: "This is your favorite", key: `favorite_${idPrefix}_id` },
    { label: "Not a fan (least favourite)", key: `least_favorite_${idPrefix}_id` },
    { label: "Great Divide", key: `most_divergent_${idPrefix}_id`, extraKey: 'max_position_difference' },
  ...
  const explainExtra = (extraKey: string | undefined, value: any) => {
    ...
    if (extraKey === 'min_position_difference') return `Min position difference: ${value}`;
    return String(value);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, marginTop: 24 }}>
      {items.map(item => {
        const title = getName(item.key);
        const normalized = type === 'albums' && typeof title === 'string' ? normalizeAlbumName(title) : undefined;
        const rawExtra = item.extraKey && analysis ? analysis[item.extraKey] : null;
        const extraText = explainExtra(item.extraKey, rawExtra);
        ...
        if (type === 'tracks') {
          return (
            <div key={item.key} style={{
              ...
              <div style={{ fontSize: 20, fontWeight: 900, color: colors.light }}>{item.label}</div>
              <div style={{ fontSize: 22, fontWeight: 900 }}>{title}</div>
              {extraText && <div style={{ fontSize: 13, opacity: 0.9 }}>{extraText}</div>}
            </div>
          );
        }
...
const MatchingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setUserTheme(themeColorMap[themeKey] || themeColorMap['MIDNIGHTS']);
  ...
  useEffect(() => {
    ...
    setPassedState((location as any).state ?? null);
  }, [location]);
  ...
  const fetchCompatibility = useCallback(async () => {
    ...
    const decision = decideEndpointFrom(queryParams, passedState);
    if ((decision as any).error) {
      setApiError('Invalid parameters.');
      return;
    }
    ...
    try {
      ...
      const headers: Record<string, string> = { 'Accept': 'application/json' };
      if (auth) headers.Authorization = auth;
      const res = await fetch(decision.url, { headers });
      ...
      if (decision.type === 'tracks' && decision.albumId) {
        fetchTracksForAlbum(decision.albumId);
      }
    } catch (e: any) { setApiError(String(e)); }
    finally { setLoading(false); }
  }, [queryParams, passedState, fetchTracksForAlbum]);

  useEffect(() => {
    if (queryParams.targetUserId || passedState?.targetUserId) fetchCompatibility();
  }, [queryParams, passedState, fetchCompatibility]);

  if (loadingTheme) return <div style={{ padding: 40 }}>Loading theme...</div>;

  const decision = decideEndpointFrom(queryParams, passedState);
  const comparisonType = decision.type as 'albums' | 'tracks';

  let colors: ThemeColor = userTheme || themeColorMap['MIDNIGHTS'];
  if (apiResponse && Object.keys(albumMap).length > 0) {
    let albumName = '';

    if (comparisonType === 'tracks' && decision.albumId) {
      albumName = albumMap[decision.albumId] || '';
    } else if (comparisonType === 'albums') {
      const albumIds = Object.values(apiResponse.matching_analysis).filter(v => typeof v === 'number') as number[];
      for (const id of albumIds) {
        if (albumMap[id]) { albumName = albumMap[id]; break; }
      }
    }

    if (albumName) {
      const normalized = normalizeAlbumName(albumName);
      if (normalized && themeColorMap[normalized]) colors = themeColorMap[normalized];
    }
  }

  const isDarkBackground = parseInt(colors.dark.substring(1, 3), 16) < 50;
  const textColor = isDarkBackground ? colors.light : colors.dark;

  return (
    <div style={{ width: '100vw', minHeight: '100vh', backgroundColor: colors.light, color: textColor, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <header style={{ width: '100%', height: HEADER_HEIGHT, background: colors.dark, display: 'flex', alignItems: 'center', paddingLeft: HEADER_GUTTER, position: 'fixed', top: 0, zIndex: 100 }}>
        <LogoSVG color={colors.light} />
      </header>

      <main style={{ paddingTop: HEADER_HEIGHT + 40, paddingBottom: 140, width: '100%', maxWidth: 960, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        {apiError && <div style={{ color: 'crimson' }}>{apiError}</div>}

        {apiResponse && (
          <>
            <h1 style={{ fontSize: 56, fontWeight: 900, color: colors.dark, textAlign: 'center', margin: 0, marginBottom: 20 }}>
              {comparisonType === 'albums'
                ? 'This is your Eras Match'
                : `This is your ${albumMap[decision.albumId!] || ''} Match!`}
            </h1>

            <div style={{ marginBottom: 30, textAlign: 'center' }}>
              <div style={{ fontSize: 64, fontWeight: 'bold', color: colors.dark }}>
                {apiResponse.compatibility_percent}%
              </div>
              <div style={{ fontSize: 22, fontWeight: 800, color: colors.dark, marginTop: 6 }}>
                Compatibility
              </div>
            </div>

            <MatchingAnalysisDisplay
              analysis={apiResponse.matching_analysis}
              type={comparisonType}
              albumMap={albumMap}
              trackMap={trackMap}
              colors={colors}
      ...
      <div style={{ position: 'fixed', bottom: 16, left: 0, right: 0, display: 'flex', justifyContent: 'center' }}>
        <button onClick={() => navigate('/catalog')} style={{ background: colors.dark, color: colors.light, borderRadius: 999, padding: '12px 18px', fontWeight: 800 }}>
          Comeback!
        </button>
      </div>
    </div>
